var vg_1 = "lollipop.vg.json";
vegaEmbed("#lollipop", vg_1).then(function(result) {
}).catch(console.error);

var vg_2 = "linechart.vg.json";
vegaEmbed("#linechart", vg_2).then(function(result) {
}).catch(console.error);

var vg_3 = "symbolmap.vg.json";
vegaEmbed("#symbolmap", vg_3).then(function(result) {
}).catch(console.error);

var vg_4 = "heatmap_bowlers.vg.json";
vegaEmbed("#heatmap_bowlers", vg_4).then(function(result) {
}).catch(console.error);

var vg_5 = "diverging_opp.vg.json";
vegaEmbed("#diverging_opp", vg_5).then(function(result) {
}).catch(console.error);

var vg_6 = "choropleth_opp.vg.json";
vegaEmbed("#choropleth_opp", vg_6).then(function(result) {
}).catch(console.error);

var vg_7 = "captains_ribbon.vg.json";
vegaEmbed("#captains_ribbon", vg_7).then(function(result) {
}).catch(console.error);

var vg_8 = "partnerships.vg.json";
vegaEmbed("#partnerships", vg_8).then(function(result) {
}).catch(console.error);

var vg_9 = "batting_extremes.vg.json";
vegaEmbed("#batting_extremes", vg_9).then(function(result) {
}).catch(console.error);
